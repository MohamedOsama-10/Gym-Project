...
import { API_CONFIG, getToken, apiUpload } from '../../services/httpClient';
...
export default function AdminProfile() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await apiUpload('/admin/profile/', formData, 'PUT');
  ...
  const handlePasswordChange = async () => {
    ...
    if (passwords.new.length < 8) {
      showToast('Password must be at least 8 characters, including an uppercase letter, a number, and a special character.', 'error'); return;
    }
    setSavingPwd(true);
    try {
  ...
  const inputCls = "w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none text-gray-900 dark:text-white transition";
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-3xl shadow-2xl p-4 md:p-8">
        ...
        <div className="relative z-10 flex items-center gap-4 md:gap-5">
          ...
          <div>
            ...
            <p className="text-blue-100">Manage your personal and gym information</p>
          </div>
        </div>
      </div>
        ...
        <>
            ...
            <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="relative">
                  {photoPreview ? (
                    <img src={photoPreview} alt="Profile" className="w-32 h-32 rounded-full object-cover border-4 border-blue-500 shadow-lg" />
                  ) : (
                    <div className="w-32 h-32 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-5xl font-black shadow-lg">
                      {(profile.full_name || 'A')[0].toUpperCase()}
                    </div>
                  )}
                  <label htmlFor="admin-photo-upload" className="absolute bottom-0 right-0 w-10 h-10 bg-blue-600 hover:bg-blue-700 rounded-full flex items-center justify-center cursor-pointer shadow-lg transition">
                    <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                  </label>
                  <input id="admin-photo-upload" type="file" accept="image/*" onChange={handlePhotoChange} className="hidden" />
                </div>
              ...
              <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-5">
                {[
                  ...
                  { label: '✉️ Email',         key: 'email',        type: 'email',  placeholder: 'admin@gym.com' },
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 p-4 md:p-8">
            <button onClick={handleSave} disabled={saving}
              className="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-semibold hover:from-blue-700 hover:to-indigo-700 transition disabled:opacity-50 shadow-lg flex items-center gap-2">
