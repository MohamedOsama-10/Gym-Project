...
def get_customer_profile(
    ...
    try:
        ...
        if current_user.phone and not customer.phone:
            ...
            needs_update = True
            logger.debug(f"📞 Syncing phone from users to customers: '{customer.phone}'")

        # Sync full_name
        if current_user.full_name and current_user.full_name != customer.full_name:
            customer.full_name = current_user.full_name
            needs_update = True
            logger.debug(f"👤 Syncing full_name to customers: '{customer.full_name}'")

        # Sync email
        if current_user.email != customer.email:
            customer.email = current_user.email
            needs_update = True
            logger.debug(f"📧 Syncing email to customers: '{customer.email}'")

        # Commit if any updates were made
        if needs_update:
            db.commit()
            db.refresh(customer)
            logger.info("✅ Synced user data to customer profile")
...
def update_customer_profile(
    ...
    try:
        ...
        if profile_data.phone is not None:
            ...
            logger.debug(f"📞 Updated phone in both users and customers tables: '{profile_data.phone}'")
